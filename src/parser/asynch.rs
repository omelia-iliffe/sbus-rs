    ...
    R: Read,
{
    ...
    pub async fn read_frame(&mut self) -> Result<SbusPacket, SbusError> {
        ...
        let header = buffer[0];
        let footer = buffer[SBUS_FRAME_LENGTH - 1];

        // Check header and footer
        if buffer[0] != SBUS_HEADER {
            return Err(SbusError::InvalidHeader(header));
        } else if buffer[SBUS_FRAME_LENGTH - 1] != SBUS_FOOTER {
            return Err(SbusError::InvalidFooter(footer));
        }
